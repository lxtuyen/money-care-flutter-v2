...
class _ExpiredFundsScreenState extends State<ExpiredFundsScreen> {
  ...
  List<SavingFundEntity> get _expiredFunds {
    final now = DateTime.now();
    return controller.savingFunds
        .where((f) => f.end_date != null && f.end_date!.isBefore(now))
        .toList()
  ...
  Future<void> _viewReport(SavingFundEntity fund) async {
    await controller.loadFundReport(fund.id);
    Get.toNamed(RoutePath.savingFundReport, arguments: fund.id);
  }

  Future<void> _extend(BuildContext context, SavingFundEntity fund) async {
...
class _ExpiredFundCard extends StatelessWidget {
  ...
  int get _daysSinceExpired {
    if (fund.end_date == null) return 0;
    return DateTime.now().difference(fund.end_date!).inDays;
  }
                ...
                Row(
                  children: [
                    _DateChip(
                      ...
                      label: 'Bắt đầu',
                      value: fund.start_date != null
                          ? date.format(fund.start_date!)
                          : '—',
                    ...
                    _DateChip(
                      ...
                      label: 'Kết thúc',
                      value: fund.end_date != null
                          ? date.format(fund.end_date!)
                          : '—',
                ...
                Row(
                  children: [
                    ...
                    _StatItem(
                      ...
                      label: 'Ngân sách',
                      value: fund.budget != null
                          ? currency.format(fund.budget)
                          : '—',
